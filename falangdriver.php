			...
			foreach ($fields as $field)
			{
				...
				if (!empty($value)) {
					...
					$query->select($db->qn('value'))
						...
						->where($db->q($db->qn('item_id'). ' = ' . $query->q($reference_id)))
						->where($db->qn('field_id') . ' = ' . $query->q($field->id));
					$db->setQuery($query);
					$joomlaValue = $db->loadObject();
...
/*
		...
		foreach ($fields as $field) {
			//only support Text field yet //type text here and Text in render why ??
			...
			$query->select($query->qn('id'))
				...
				->where($db->qn('reference_id') . ' = ' . $db->q($field->id))
				->where($db->qn('reference_field') . ' = ' . $db->q('value'))
				->where($db->qn('reference_table') . ' = ' . $db->q('fields_values'));
			...
			if (isset($value)){
				...
				//delete the previous value if null
				$query = $db->getQuery(true);
				$query->delete($db->qn('#__falang_content'))
					->where($db->qn('language_id') . ' = ' . $db->q($language_id))
					->where($db->qn('reference_id') . ' = ' . $db->q($field->id))
					->where($db->qn('reference_field') . ' = ' . $db->q('value'))
					->where($db->qn('reference_table') . ' = ' . $db->q('fields_values'));
